	...
	public List<RecruitmentDTO> recruitmenTopList() {
		...
		try {
			String sql = " select rank, comName, jobName, case when length(title) > 12 then substr(title, 1, 10) || '..' else title end AS title, emp_type, experience, to_char(deadlineday, 'yyyy-mm-dd') AS deadlineday, is_delete "
					   + " from  "
					   + " ( "
					   + " select dense_rank()over(order by(cnt) desc) AS rank, comName, jobName, title, emp_type, experience, deadlineday, is_delete, cnt "
					   + " from "
					   + "     ( "
					   + "         select  c.name AS comName, j.name AS jobName, R.title, emp_type, experience, deadlineday, is_delete, nvl(cnt, 0) AS cnt "
					   + "         from "
